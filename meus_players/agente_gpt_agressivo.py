...
# --- 2. Função para Consultar o GPT (Estilo Agressivo) ---
def consultar_gpt_agressivo(prompt_detalhado):
    """
    Envia o estado do jogo para o GPT com instruções para ser agressivo.
    """
    ...
    try:
            ...
            messages=[
                {
                    ...
                    "content": "Você é um jogador de poker 'Loose Aggressive' (LAG). Seu objetivo é dominar a mesa com agressividade. Você joga muitas mãos e aposta/aumenta com frequência para colocar pressão nos oponentes. Blefar faz parte do seu jogo. Forneça sua decisão no formato 'ação, valor'. Ex: 'raise, 50'."
                },
                {"role": "user", "content": prompt_detalhado}
            ],
            temperature=0.8, # Aumentamos a "criatividade" para encorajar mais blefes e jogadas não-padrão.
            max_tokens=15
        )
        
        resposta_gpt = completion.choices[0].message.content.strip().lower()
        print(f"--- Resposta Bruta do GPT (Agressivo): '{resposta_gpt}' ---")
        ...
        if 'raise' in resposta_gpt: return 'raise', 0 # Prioriza o raise se o formato falhar
...
# --- 3. Classe do Agente Agressivo ---
class AgenteAgressivo(BasePokerPlayer):

    def declare_action(self, valid_actions, hole_card, round_state):
        print(f"\n--- MÃO DO AGENTE AGRESSIVO: {hole_card} ---")
        ...
        **Decisão Necessária:**
        Lembre-se da sua personalidade agressiva. Coloque pressão.
        ...
        print(f"--- AGENTE AGRESSIVO: Consultando o LLM... ---")
        acao_recomendada, valor_recomendado = consultar_gpt_agressivo(prompt)

        if acao_recomendada == 'raise' and raise_action_info['amount']['min'] != -1:
            acao = raise_action_info['action']
            # Se o GPT sugerir um valor de raise muito baixo, vamos pelo menos aumentar o mínimo
            valor_base = max(raise_action_info['amount']['min'], valor_recomendado)
            # Garante que o valor está dentro dos limites
            valor = min(raise_action_info['amount']['max'], valor_base)
            print(f"AGENTE AGRESSIVO: Decisão final -> {acao} {valor}")
            return acao, valor
